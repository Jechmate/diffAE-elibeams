...
def sample_all(root="models", result_dir="results/transfer_withgain_512_valid", device='cuda:2', n=8, dataset=Path("data/with_gain")):
    ...
    for subdir in tqdm(dir_list):
        ...
        model.eval()
        diffusion = SpacedDiffusion(beta_start=1e-4, beta_end=0.02, section_counts=[30], noise_steps=700, img_width=128, img_height=64, device=device)
        ...
        while total != original_size:
            if total + n > original_size:
                ...
                add = n
            x = diffusion.ddim_sample_loop(model, y, cfg_scale=3, resize=[256, 512], n=add, eta=1, device=device)
