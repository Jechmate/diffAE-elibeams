% Script to make pointing for Figure 2

software_folder=pwd; % save folder where the script is located
% data_folder_address = uigetdir('','Select Electron Data Folder');
data_folder_address = ['/Users/matejech/Work/elibeams/Electron_Spectra_ALFA/22']
%----Initialization and User Inputs-----------
fprintf(data_folder_address)
cd(data_folder_address); % Go to data folder
images=dir('*.tiff'); % read all the images in the folder
%[BKG_image_name, BKG_image_path]=uigetfile({'*.tiff'},'Select Background Image'); % select BKG image
%cd(BKG_image_path);
%BKG_image=imread(BKG_image_name);
%cd(data_folder_address);
%BKG_gray=mat2gray(BKG_image);

% Ticks for images at -5, 0 and +5 mrad
%tick_5mrad_px= laser_pointing_ver_cropped - round(5/pixel_in_mrad);
%tick0mrad_px= laser_pointing_ver_cropped;
%tick5mrad_px = laser_pointing_ver_cropped + round(5/pixel_in_mrad);
...
black_dot_center_1=[53 60]; % Here indicate the #1 black dot center in pixel in the cropped image as written by MATLAB datatip
...
black_dot_center_2=[123 58];
black_dot_radius_2=12;
black_dot_center_3=[186 56];
black_dot_radius_3=12;
black_dot_center_4=[273 59];
...
for i=15:15
            ...
            for m=1:ver_image_size
                if l<center_hor+radius && m<center_ver+radius && l>center_hor-radius  && m>center_ver-radius ...
                    ...
                    I_wo_black_dots(m,l)=((aux_x_max-l)/(aux_x_max-aux_x_min))*I_crop(m,aux_x_min)+...
                                         ...
                                         %((aux_y_max-m)/(aux_y_max-aux_y_min))*I_crop(aux_y_min,l)+...
                                         %((m-aux_y_min)/(aux_y_max-aux_y_min))*I_crop(aux_y_max,l);
                    %I_wo_black_dots(m,l)=I_wo_black_dots(m,l)/2;
   ...
   %xline(tick5MeV,'--w','LineWidth',2)
   %xline(tick3MeV,'--w','LineWidth',2)
...
I_zoomed=I_calibrated(:,1:tick10MeV+10);
 ...
 figure(11)
   title('Zoomed Image')
    ...
    %clims=[noise_calibrated*1000 max_colorbar];
    h.FontSize = 48;
...
% figure(12)
%    title('Zoomed Image to save')
%    clims=[noise_calibrated max_colorbar]; 
%    imagesc(I_zoomed*1000,clims)
%    colormap(turbo)
%     ylabel('mrad')
%     xlabel('MeV')
%     colorbar
...
%     max_colorbar=h.Limits(2);
%     h.FontSize = 48;
%     h.Ticks = [noise_calibrated+(max_colorbar-noise_calibrated)*0.2  noise_calibrated+(max_colorbar-noise_calibrated)*0.6 max_colorbar ];
%     h.TickLabels = [round(noise_calibrated+(max_colorbar-noise_calibrated)*0.2)...
%         round(noise_calibrated+(max_colorbar-noise_calibrated)*0.6) round(max_colorbar)];
%     h.TickLength = 0.05;
%     h.LineWidth=1.5;
%     ylabel(h, 'pA/msr')
%    yline(tick_10mrad_px,'--k','LineWidth',2)
%    yline(tick10mrad_px,'--k','LineWidth',2)
%    xline(electron_pointing_pixel,'r','LineWidth',2)
%    xline(tick50MeV,'--k','LineWidth',2)
%    xline(tick30MeV,'--k','LineWidth',2)
%    xline(tick20MeV,'--k','LineWidth',2)
%    xline(tick10MeV,'--k','LineWidth',2)
%    
%    yt=[tick_10mrad_px tick0mrad_px tick10mrad_px];
%    xt=[  tick50MeV tick30MeV tick20MeV tick10MeV];  
%    set(gca, 'XTick',xt, 'XTickLabel',{'50','30','20','10'}, ...
%   'YTick',yt,'YTickLabel',{'-10','0','10'},'Fontsize',48)   % Label Ticks
  ...
  % figure(2)
  % plot(horizontal_divergence_fit,hor_pixels,horizontal_profile)
  % figure(3)
  % plot(vertical_divergence_fit,ver_pixels,vertical_profile)
...
%{
figure(4)
   title('Average Pointing')
    %clims = [0 2];
    imagesc(average_image_calibrated)
    ylabel('mrad')
    colorbar
    h = colorbar;
    ylabel(h, 'nA/msr')
   xline(45,'--w','LineWidth',3)
   xline(140,'--w','LineWidth',3)
   xline(60,'r','LineWidth',2)
  yt=[tick_10mrad_px tick0mrad_px tick10mrad_px];
    xt=[ laser_pointing_hor_cropped tick40MeV tick20MeV tick10MeV tick5MeV tick3MeV];  
    set(gca, 'XTick',xt, 'XTickLabel',{'INF','40','20','10','5','3'}, ...
    'YTick',yt,'YTickLabel',{'-10','0','10'},'fontsize',18)   % Label Ticks
...
horizontal_profile=sum(average_image);
    ...
    %plot(spectrum_fit, deflection_MeV_for_fit,spectrum_calibrated_for_fit,'*')
...
rms_spectrum_mean_plot=rms_spectrum_mean(electron_pointing_pixel+15:end)';
 %{   
    figure(10)
    p=plot(deflection_MeV,spectrum30bar,deflection_MeV,spectrum34bar,...
        deflection_MeV,spectrum38bar,deflection_MeV,spectrum10_5bar)
    p(1).LineWidth=2;
    p(2).LineWidth=2;
    p(3).LineWidth=2;
    p(4).LineWidth=2;
    legend('n_e=0.072n_{cr}','n_e=0.067n_{cr}','n_e=0.063n_{cr}','n_e=0.058n_{cr}')
    axis([5 40 0 5])
    ylabel('Spectral Intensity (pA/MeV)')
    xlabel('Energy (MeV)')
    title('Average electron spectra with 0.65 TW laser pulses in N_2')
